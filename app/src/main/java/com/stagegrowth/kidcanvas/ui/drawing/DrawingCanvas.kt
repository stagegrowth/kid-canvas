...
/**
 * 재사용 가능한 드로잉 캔버스.
 * 이 컴포넌트는 상태를 가지지 않고, 입력은 콜백으로만 위로 보냄(상태 호이스팅).
 *
...
fun DrawingCanvas(
    ...
    Canvas(
        modifier = modifier.pointerInput(Unit) {
            awaitEachGesture {
                // pointerInput 람다 안에서 size 는 실제 컴포저블의 픽셀 크기(IntSize)
...
private fun DrawScope.drawStroke(stroke: Stroke, density: Density) {
    ...
    if (stroke.points.size == 2) {
        ...
        // 3개 이상: 각 중간점을 control point, 다음 점과의 중점을 endpoint 로 quadratic 보간
